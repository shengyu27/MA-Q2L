...
def parser_args():
    parser = argparse.ArgumentParser(description='MAQ2L Sewer-ML Training')
    parser.add_argument('--dataname', help='dataname', default='sewerml', choices=['sewerml'])
    parser.add_argument('--dataset_size', help='dataset_size', default=1, choices=['1', '0.0625'])

    parser.add_argument('--dataset_dir', help='dir of dataset', default='./dataset')
    ...
    parser.add_argument('--optim', default='Adam_twd', type=str, choices=['AdamW', 'Adam_twd'],
...
def main():
    ...
    if args.seed is not None:
        ...
        # for cpu set random seed in order to reproduction
        torch.manual_seed(args.seed)
        np.random.seed(args.seed)

    # design more GPUs for training
...
def main_worker(args, logger):
    ...
    criterion = lib.models.aslloss.AsymmetricLossOptimized(
        gamma_neg=args.gamma_neg, gamma_pos=args.gamma_pos, # 2 0
        clip=args.loss_clip,  # 0.0
        disable_torch_grad_focal_loss=args.dtgfl, # true
        eps=args.eps, # 1e-5
    )
    ...
    if args.evaluate:
        _, mAP, _, _ = validate(val_loader, model, criterion, args, logger)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # train for one epoch
        loss = train(train_loader, model, ema_m, criterion, optimizer, scheduler, epoch, args, logger, weight_list)
        ...
        if epoch % args.val_interval == 0:
            ...
            loss, mAP, list_output_sm, mAP_list = validate(val_loader, model, criterion, args, logger)

            loss_ema, mAP_ema, list_output_sm_ema, mAP_list_ema = validate(val_loader, ema_m.module, criterion, args, logger)

            if args.dy_weight:
                weight_list = loss_weight(mAP_list_ema)
                logger.info("loss weight top {}:".format(args.weight_top, weight_list))
            ...
            if is_best:
                ...
                save_name = 'saved_data.{'+str(best_epoch)+'}.txt'
                np.savetxt(os.path.join(args.output, save_name), list_output_sm_ema)
            ...
            # output_list = [0, 4, 9, 14, 19]
            # if epoch in output_list:
            #     save_checkpoint_v({
            #         'epoch': epoch + 1,
            #         # 'arch': args.arch,
            #         'state_dict': state_dict,
            #         # 'best_mAP': best_mAP,
            #         'optimizer': optimizer.state_dict(),
            #     }, epoch, filename=os.path.join(args.output, 'checkpoint.pth.tar'))
...
def loss_weight(mAP_list_ema):
    args = get_args()
    # calculate mAP index t for loss weight
    mAP_mean = np.mean(mAP_list_ema)
    mAP_list_ema = (mAP_mean - mAP_list_ema) / 10.0
    # pay attention to neg class which mAP is low than mean
    mAP_list_ema[mAP_list_ema < 0] = 1
    # sort
    sorted_lst = sorted(enumerate(mAP_list_ema),key=lambda x:x[1],reverse=True)
    sorted_indices = [index for index, _ in sorted_lst]
    # only top k
    top = args.weight_top
    for index in range(len(mAP_list_ema)):
        if index < top:
            continue
        else:
            mAP_list_ema[sorted_indices[index]] = 1

    print("loss weight(top {}):".format(top), mAP_list_ema)
    return mAP_list_ema



def train(train_loader, model, ema_m, criterion, optimizer, scheduler, epoch, args, logger, weight_index_list):
    # amp: scale gradients for train
    ...
    for i, (images, target) in enumerate(train_loader):
        ...
        with torch.cuda.amp.autocast(enabled=args.amp):
            ...
            loss = criterion(output, target, weight_index_list)
            if args.loss_dev > 0:
                loss *= args.loss_dev
...
@torch.no_grad()
def validate(val_loader, model, criterion, args, logger, weight_index_list=None):
    ...
    return loss_avg, mAP, list_output_sm, mAP_list
...
#
# def save_checkpoint_v(state, epoch,filename='checkpoint.pth.tar'):
#     # torch.save(state, filename)
#
#     torch.save(state, os.path.split(filename)[0]+ '/'+str(epoch)+'model_best.pth.tar')
        # shutil.copyfile(filename, os.path.split(filename)[0] + '/model_best.pth.tar')
...
def get_inf_time():
    ...
    print(f'Mean@1{mean_syn:.3f}ms  Std@5{std_syn:.3f}ms FPS@1{mean_fps:.2f}'.format(mean_syn=mean_syn,std_syn=std_syn,mean_fps=mean_fps) )
    print(mean_syn)
